 ...
 * pattern is industry-standard (Stripe Checkout, Firebase Console, every
 * banking app) precisely because async flows that touch auth or
 * destructive state MUST be modal until they resolve.
 *
 * Usage:
 *
 * ```
 ...
 *
 * The overlay:
 *   - Fades in/out (200ms) — feels less abrupt than a hard cut
 ...
 *
 * Edge cases:
 *   - Back-press: the overlay does NOT swallow the back button. Caller
 *     screens should disable back via [BackHandler] separately if they
 *     don't want the user to escape an in-flight call (typically auth
 *     ops should disable back; non-destructive saves can allow it).
 *   - System dialogs / IME: this overlay sits ABOVE LazyColumn content
 *     but BELOW system UI (dialogs, IME, sheets) since it's just a
 *     composable in the same window. That's the right layering — a
 *     loading overlay should not block an emergency system dialog.
 */
@Composable
fun LoadingOverlay(
    visible: Boolean,
